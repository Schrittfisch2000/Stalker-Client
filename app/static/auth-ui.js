(() => {
  ...
  async function openAdmin(current, initialPanel = 'users') {
    ...
    dialog.querySelector('.user-note').textContent = `Angemeldet als ${current.username} (Administrator)`;
    await renderUsers(dialog, current);
  ...
  function hidePortalControlsForUser() {
    const settingsButton = document.getElementById('openSettings');
    const heroSettings = document.getElementById('heroSettings');
    const settingsDialog = document.getElementById('settingsDialog');
    for (const element of [settingsButton, heroSettings]) {
      if (!element) continue;
    ...
    if (settingsDialog) {
      ...
      settingsDialog.classList.add('admin-only-hidden');
      settingsDialog.querySelectorAll('input, button').forEach((element) => { element.disabled = true; });
      settingsDialog.querySelectorAll('input').forEach((input) => { input.value = ''; });
    }
  }
  ...
  async function init() {
    ...
    if (!isAdmin) hidePortalControlsForUser();
    ...
    userButton.title = isAdmin ? 'Administration öffnen' : 'Angemeldeter Benutzer';
    if (isAdmin) userButton.onclick = () => openAdmin(status.user, 'users');
